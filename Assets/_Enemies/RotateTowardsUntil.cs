...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
	...
	public class RotateTowardsUntil : Action
	{
        ...
        [Tooltip("Time Duration for rotating towards rotating towards")]
        public SharedFloat rotateDuration;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            //}
            if(timer >= rotateDuration.Value)
            {
                timer = 0;
                return TaskStatus.Success;
            }
            // We haven't reached the target yet so keep rotating towards it
            transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, maxLookAtRotationDelta.Value * Time.deltaTime);
